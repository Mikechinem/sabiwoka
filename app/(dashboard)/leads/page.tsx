...
import { Plus, X, Users, Edit3, UserPlus } from "lucide-react";
import { useLeads } from "@/hooks/useLeads"; // Keeping the hook name for now, but UI says Customers
    ...
    <div className="max-w-md mx-auto px-4 pt-20 pb-28">
      ...
      <div className="flex items-center justify-between mb-8">
        ...
        <div className="text-right">
           ...
           <p className="text-sm font-bold text-[#134e4a]">{activeLeads.length} Potential</p>
        </div>
      </div>
      ...
      <div className="relative flex bg-gray-200/80 p-1.5 rounded-2xl mb-8 border border-gray-300/20 shadow-inner">
        {(["active", "closed"] as const).map((tab) => (
          ...
          >
            {tab === "active" ? "Active" : "Past"} {tab === "active" ? `(${activeLeads.length})` : `(${closedLeads.length})`}
          </button>
        ))}
      </div>
      ...
      {/* List */}
      <div className="mt-2">
        <h2 className="font-bold text-gray-800 text-sm uppercase tracking-wider mb-4 px-1">{activeTab} Customers</h2>
        {loading ? (
          ...
          <div className="text-center py-16 bg-white rounded-3xl border border-dashed border-gray-200">
            ...
            <p className="text-gray-400 text-sm font-medium">No {activeTab} customers found. 🚀</p>
          </div>
        ) : (
        ...
        {showActionMenu && (
          <div className="fixed inset-0 z-[100] bg-black/60 flex items-end sm:items-center justify-center p-4 backdrop-blur-sm">
            <motion.div 
              ...
              className="bg-white w-full max-w-md rounded-[2.5rem] p-6 shadow-2xl pb-10 sm:pb-6"
            >
               <div className="flex justify-between items-center mb-6">
                  <h2 className="font-black text-lg text-gray-900">New Customer</h2>
                  <button onClick={() => setShowActionMenu(false)} className="p-2 bg-gray-100 rounded-full text-gray-400">
                    <X size={20}/>
                  </button>
               </div>
               ...
               <div className="space-y-4">
                  <button 
                    onClick={() => { setShowActionMenu(false); setShowForm(true); }} 
                  ...
                  <div className="relative flex items-center py-2">
                    ...
                    <span className="flex-shrink-0 mx-4 text-gray-300 text-[10px] font-black uppercase tracking-widest">Or Use AI</span>
                    <div className="flex-grow border-t border-gray-100"></div>
                  </div>

                  <div className="space-y-3">
                    <div className="px-1">
                      <p className="text-[10px] uppercase font-black text-gray-400 tracking-widest mb-2">Record Voice</p>
                      <LeadVoiceRecorder /> 
                    </div>
                    <div className="px-1">
                      <p className="text-[10px] uppercase font-black text-gray-400 tracking-widest mb-2">WhatsApp Paste</p>
                      <MagicPasteBox />
                    </div>
                  </div>
               </div>
      ...
      <AnimatePresence>
        {showForm && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[110] flex items-end justify-center px-4 pb-6 bg-black/40 backdrop-blur-sm">
            <motion.div initial={{ y: 80, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 80, opacity: 0 }} className="w-full max-w-md bg-white rounded-3xl p-6 max-h-[85vh] overflow-y-auto shadow-2xl">
              <div className="flex items-center justify-between mb-5">
                <h2 className="font-black text-gray-900 uppercase text-xs tracking-widest">Customer Details</h2>
              ...
              <div className="space-y-3">
                {[
                  ...
                  { key: "item_of_interest", label: "Interested in...", type: "text" },
                  { key: "amount", label: "Proposed Price (₦)", type: "number" },
                  { key: "notes", label: "Notes / Preferences", type: "text" },
                ].map(({ key, label, type }) => (
                  <div key={key}>
