...
export default function DashboardLayout({
  ...
  useEffect(() => {
    async function getProfile() {
      ...
      if (user) {
        // 1. Try metadata (from signup form)
        // 2. Fallback to display name
        // 3. Last resort: split the email
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-20">
      ...
      <header className="fixed top-0 left-0 right-0 z-40 h-16 px-4 bg-white/80 backdrop-blur-md border-b border-gray-100 flex items-center justify-between">
        ...
        {/* Settings Trigger with Gear Badge */}
        <motion.button 
          whileTap={{ scale: 0.95 }}
      ...
      {/* Main Content Area - pt-4 allows children's internal pt-20 to stack correctly */}
